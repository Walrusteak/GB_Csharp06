using Homework03.Models.Repository;
...
namespace Homework03.Repositories
{
    public class PersonRepository : IRepository<Person>
    {
        ...
        public void Add(Person entity) => _data.Add(entity);

        public void Delete(int id) => Delete(_data.First(p => p.Id == id));

        public void Delete(Person entity) => _data.Remove(entity);

        public Person Get(int id) => _data.First(p => p.Id == id);

        public IEnumerable<Person> Get(int skip, int take) => _data.Skip(skip).Take(take).ToList();

        public IEnumerable<Person> Search(string term) => _data.Where(p => p.FirstName.Contains(term) || p.LastName.Contains(term)).ToList();

        public void Update(int id, Person entity) => _data[_data.FindIndex(p => p.Id == id)] = entity;
    }
}
